...
def test_no_in_memory_state_survives(restarted):
    ...
    assert len(data["ava"]) == 5 and len(data["noah"]) == 4
...
def test_seq_is_per_character_and_dense(restarted):
    ...
    assert [m["seq"] for m in data["warren"]] == [0, 1, 2, 3]
    assert [m["seq"] for m in data["noah"]] == [0, 1, 2, 3]
...
def test_canonical_holds_more_than_anyone_perceived(restarted):
    ...
    assert world.event_count() == 6
    assert max(len(v) for v in data.values()) <= 6
